...
function docReady(fn) {
  // veja se DOM já está disponível
  if (
    document.readyState === "complete" ||
    document.readyState === "interactive"
  ) {
    // ligue no próximo tick disponível
    setTimeout(fn, 1);
  } else {
...
docReady(async function () {
  ...
  //primeira vez que a página é carregada: tente obter o status da conexão e inicie a verificação de wi-fi
  await refreshAP();
  startCheckStatusInterval();
...
async function performConnect(conntype) {
  //pare a atualização do status. Isso evita uma condição de corrida em que um status
  //a solicitação seria atualizada com informações de ip incorretas de uma conexão anterior
  //e a solicitação seria mostrada automaticamente como bem-sucedida.
  stopCheckStatusInterval();

  //pare de atualizar a lista de wi-fi
  stopRefreshAPInterval();

  var pwd;
  if (conntype == "manual") {
    //Pegue o SSID manual e o PWD
  ...
  //agora podemos redefinir os intervalos, independentemente do resultado
  startCheckStatusInterval();
  startRefreshAPInterval();
}
...
async function refreshAP(url = "ap.json") {
  try {
    ...
    if (access_points.length > 0) {
      //classificar pela intensidade do sinal
      ...
      if (data["ssid"] === selectedSSID) {
        ...
        switch (data["urc"]) {
          case 0:
            ...
            //desbloquear a tela de espera se necessário
            gel("ok-connect").disabled = false;

            //tela de espera de atualização
          ...
          case 1:
            ...
            //não mostra nenhuma conexão
            gel("wifi-status").display = "none";

            //desbloquear a tela de espera
            gel("ok-connect").disabled = false;
        ...
        //ESP32 já está conectado a um wi-fi sem que o usuário faça nada
        if (
